            ...
            if (inputData.keyCode >= ROT.VK_A && inputData.keyCode <= ROT.VK_Z) {
                ...
                if (this._options[index]) {
                    ...
                    if (this._entity.getStatPoints() == 0) {
                        Game.Screen.levelSkillsScreen.setup(this._entity);
                        Game.Screen.playScreen.setSubScreen(Game.Screen.levelSkillsScreen);
                    } else {
                        Game.refresh();
                    }
                }
...
Game.Screen.levelSkillsScreen = {
    setup: function(entity){
        this._entity = entity;
        this._options = entity.getPassives();
    },
    render: function(display){
        ...
        // Render remaining stat points
        display.drawText(0, 4 + this._options.length,
            "Remaining points: " + this._entity.getSkillPoints());   
    },
    handleInput: function(inputType, inputData) {
        if (inputType === 'keydown') {
            // If a letter was pressed, check if it matches to a valid option.
            if (inputData.keyCode >= ROT.VK_A && inputData.keyCode <= ROT.VK_Z) {
                ...
                if (this._options[index]) {
                    // Call the stat increasing function
                    this._entity.increaseSkill(this._options[index], 1);
                    // Decrease stat points
                    this._entity.setSkillPoints(this._entity.getSkillPoints() - 1);
                    // If we have no stat points left, exit the screen, else refresh
                    if (this._entity.getSkillPoints() == 0) {
